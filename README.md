...
Конструктор не принимает параметров. Инициализирует модель с пустыми полями.
Поля класса:
```
private payment: TPayment | null - способ оплаты
private address: string - адрес доставки
private phone: string - номер телефона
private email: string - адрес электронной почты
```
Методы класса:
```
setBuyerData(data: Partial<IBuyer>): void - сохраняет данные покупателя. Позволяет обновить как все поля сразу, так и частично
getBuyerData(): IBuyer - возвращает все данные покупателя в виде объекта
clear(): void - сбрасывает все данные покупателя к значениям по умолчанию
validate(): { [field in keyof IBuyer]?: string } - выполняет валидацию данных и возвращает объект с ошибками
```
